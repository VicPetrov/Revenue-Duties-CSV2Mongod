...
class Document(BaseModel):
    ...
    def doc_string_date_2_datetime(cls, v):
        if isinstance(v, str):
            captured_day, captured_month, captured_year = 0, 0, 0
            for match in re.finditer(settings.document_date_format, v):
    ...
    def dates_check(cls, values):
        try:
            ...
            _date_ended = cls.doc_string_date_2_datetime(v=_date_ended, cls = cls)
            _date_started = cls.doc_string_date_2_datetime(v=_date_started, cls = cls)
            if _date_ended is not None and _date_started is not None and (_date_started > _date_ended):
                raise ValueError(settings.document_date_check_failed_msg.format(**{"ds":_date_started, "de":_date_ended}))
            else:
