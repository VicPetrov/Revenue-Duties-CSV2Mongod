...
import models.logic.validators
...
class Declaration(Paper):
    ...
    declaration_id: str = Field(...,
                                regex=settings.declaration_id_pattern,
    ...
    reciever: Optional[Entity] = Field(None, title="Reciever Entitty")
    reciever_additional: Optional[Entity] =  Field(None, title="Additional Reciever Entitty")
    ...
    country_of_trade: Optional[CountryOfTrade] = Field(None, title="Country of Trade")
    value_in_rub: Union [float, str, None] = Field(None, regex=settings.decimal_places_regex, title="Value in Roubles")
    country_of_departure: Optional[Country] = Field(None, title="Country of Departure")
    country_of_origin: Optional[CountryOfOrigin] = Field(None, title="Country of Origin")
    ...
    def validate_crud_type(cls, v):
        if v.startswith("m"):
            ...
            for sk, dv in settings.declaration_crud_allow_dict.items():
                if v in dv:
                    return sk
        raise ValueError()
    ...
    def validate_id(cls, v):
        m = re.search(pattern="".join(settings.declaration_id_pattern),
                     string=v)
        if m is not None:
            validators.validate_authority(regex_match=m,
                                          allow_list=settings.authority_allow_list,
                                          msg="".join(settings.validate_id_authority_msg
                                                      ).format(
        ...
        try:
            validators.date_check(regex_match=m, msg=settings.validate_id_msg.format(**{"date":".".join([m.group("day"), m.group("month"), m.group("year")]), "v":v}))
        finally:
            return v
    ...
    @validator("reciever_additional")
    def reciever_is_none(cls, v, values):
        if v is not None and values["reciever"] is None:
            raise ValueError(settings.reciever_is_none_msg
            )
        return v
    ...
    def decl_root(cls, values):
        try:
            ...
            if isinstance(values["country_of_trade"], dict):
                try:
                    values["country_of_trade"] = CountryOfTrade(**values["country_of_trade"])
                except Exception as e: 
                    raise ValueError("Страна продавец: \n\t\t"+str(e))
            if isinstance(values["country_of_origin"], dict):
                try:
                    values["country_of_origin"] = CountryOfOrigin(**values["country_of_origin"])
                except Exception as e:
